...
const [InternData,setInternData]=useState([])
  ...
  useEffect(()=>{
    const fetchData= async()=>{
        try {
        const response= await axios.get(`https://internarea-backend-jbx2.onrender.com/api/internship`)
  ...
  const filterInterships = (category, location) => {
    if (InternData && InternData.length > 0) {
        ...
        const filterData = InternData.filter(
          (internship) =>{
         return internship.category.toLowerCase().includes(category.toLowerCase()) && internship.location.toLowerCase().includes(location.toLowerCase())
          }
        );
...
<p id='filter-ico' className=' text-center' ><i onClick={showDiv} className="bi bi-funnel  text-blue-400"></i> Filter</p>
